...
app.post("/api/generate", async (req, res) => {
  ...
  try {
    ...
    const response = await generateText({
      model: openai("gpt-4o-mini"),//model: openai("gpt-4.1"), //
      system: SYSTEM,
      prompt,
      tools,
    });
    ...
    const outPath = path.join(
      testsDir,
      filename.endsWith(".spec.js") ? filename : `${filename}.spec.js`
    );
...
async function runPlaywright(specFile?: string, grep?: string) {
  return new Promise((resolve, reject) => {
    ...
    exec(
      cmd,
      { env: { ...process.env, FORCE_COLOR: "0" }, timeout: 60000 },
      (err, stdout, stderr) => {
        ...
        exec(
          ...
          (allureErr, allureStdout, allureStderr) => {
            return resolve({ ok: true, stdout, stderr, reportPath: reportDir });
          }
        );
      }
    );
